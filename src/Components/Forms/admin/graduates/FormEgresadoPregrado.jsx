...
import { addGraduatesPregrado } from "../../../../services/admin/graduatesService";
...
export function FormEgresadoPregrado() {
    const dispatch = useDispatch();
    const { egresadosPregrado } = useSelector((state) => state.manageGraduates);
    ...
    const handleSubmit = (e) => {
        ...
        if (!file) {
            ...
            dispatch(addGraduatesPregrado(formData))
        }
    };

    const hasErrors = egresadosPregrado?.errorDetails?.length > 0;
                ...
                <form className="flex flex-col gap-5 border-b-2 border-verdeD pb-8">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <FileInput
                            ...
                            accept=".csv"
                        />
                        <CsvFormatExample type="pregrado" />
                    </div>

                    <ButtonSmall
                        action={handleSubmit}
                        className={"bg-verdeA hover:bg-RojoC"}
                        text={"GUARDAR EGRESADOS DE PREGRADO"}
                    />
                </form>

                {/* Sección de resultados */}
                {egresadosPregrado?.total && (
                    <div className="mt-6 p-4 bg-Gris rounded-lg">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <div className="bg-white p-3 rounded shadow">
                                ...
                                <p className="text-xl font-bold">{egresadosPregrado.total}</p>
                            </div>
                            <div className="bg-white p-3 rounded shadow">
                                <p className="text-sm text-gray-500">Insertados</p>
                                <p className="text-xl font-bold text-verdeB">{egresadosPregrado.inserted}</p>
                            </div>
                            <div className="bg-white p-3 rounded shadow">
                                <p className="text-sm text-gray-500">Duplicados</p>
                                <p className="text-xl font-bold text-yellow-400">{egresadosPregrado.duplicates}</p>
                            </div>
                            <div className="bg-white p-3 rounded shadow">
                                <p className="text-sm text-gray-500">Errores</p>
                                <p className="text-xl font-bold text-RojoC">{egresadosPregrado.validationErrors + egresadosPregrado.dbErrors}</p>
                            </div>
                        </div>
            ...
            <ModalNotHeader
                ...
                component={
                    <ThemeProvider theme={customTheme}>
                        <ErrorsDetailsModal errors={egresadosPregrado?.errorDetails || []} />
                    </ThemeProvider>
                }
            />
        </>
